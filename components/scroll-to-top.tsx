...
export function ScrollToTop() {
  ...
  useEffect(() => {setIsMounted(true); scrolledPathnameRef.current = pathname;}, []);
    ...
    if (hash) {
      setTimeout(() => {
        try {
          ...
          if (shouldScrollToTop) { window.scrollTo({ top: 0, behavior: 'smooth' }); scrolledPathnameRef.current = pathname; }
        }
      }, 100);
    } else if (shouldScrollToTop) {
      window.scrollTo({ top: 0, behavior: 'smooth' });
