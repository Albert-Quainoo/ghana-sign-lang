...
const mockSignImages: Record<string, string> = {
  hello: "/placeholder.svg?height=100&width=100&text=Hello", thank: "/placeholder.svg?height=100&width=100&text=Thank", you: "/placeholder.svg?height=100&width=100&text=You", please: "/placeholder.svg?height=100&width=100&text=Please", help: "/placeholder.svg?height=100&width=100&text=Help", learn: "/placeholder.svg?height=100&width=100&text=Learn", sign: "/placeholder.svg?height=100&width=100&text=Sign", language: "/placeholder.svg?height=100&width=100&text=Language", good: "/placeholder.svg?height=100&width=100&text=Good", morning: "/placeholder.svg?height=100&width=100&text=Morning", afternoon: "/placeholder.svg?height=100&width=100&text=Afternoon", evening: "/placeholder.svg?height=100&width=100&text=Evening", name: "/placeholder.svg?height=100&width=100&text=Name", what: "/placeholder.svg?height=100&width=100&text=What", how: "/placeholder.svg?height=100&width=100&text=How", are: "/placeholder.svg?height=100&width=100&text=Are", welcome: "/placeholder.svg?height=100&width=100&text=Welcome", sorry: "/placeholder.svg?height=100&width=100&text=Sorry", yes: "/placeholder.svg?height=100&width=100&text=Yes", no: "/placeholder.svg?height=100&width=100&text=No",
};

const fallbacks = {
    title: "Ghanaian Sign Language Translator", subtitle: "Convert text to Ghanaian Sign Language signs to help bridge communication gaps.", inputTitle: "Text to GSL", inputSubtitle: "Enter text below to translate it into Ghanaian Sign Language signs. Our translator supports common words and phrases.", inputPlaceholder: "Enter text to translate to GSL...", translateButton: "Translate to GSL", translating: "Translating...", resultTitle: "Translation Result", resultEmpty: 'Enter text above and click "Translate to GSL" to see the sign language translation.', resultTip: 'Try simple phrases like "Hello, how are you?" or "Thank you for your help."', recent: "Recent Translations", supported: "Supported Words", supportedDesc: "Our translator currently supports basic words like: hello, thank you, please, help, learn, sign, language, good morning/afternoon/evening, etc.", supportedNote: "We're continuously expanding our sign database to include more words and phrases.", note: "Note: Some words may not have corresponding signs in our database yet.", learnTitle: "Want to Learn GSL?", learnDesc: "Our translator is a helpful tool, but learning GSL directly will enable you to communicate more effectively.", learnCta: "Start Learning GSL", familyCta: "Family Resources", signAlt: "Sign language sign illustration", loading: "Loading...", standardMode: "Standard Mode", highContrast: "High Contrast", stop: "Stop", readPage: "Read Page", readPageInstruction: "Welcome to the Ghanaian Sign Language Translator. Type text in the box below to translate it to sign language.", readSigns: "Read Signs Aloud", learnCard1Title: "Structured Courses", learnCard1Desc: "Follow our comprehensive curriculum designed by GSL experts, with video lessons and interactive exercises.", learnCard1Link: "View Courses", learnCard2Title: "Community Practice", learnCard2Desc: "Connect with other learners and native GSL users to practice your skills in a supportive environment.", learnCard2Link: "Join Community", learnCard3Title: "Interactive Dictionary", learnCard3Desc: "Access our comprehensive GSL dictionary with visual demonstrations of signs and detailed explanations.", learnCard3Link: "Explore Dictionary",
};
...
export default function TranslatorPage() {
  ...
  const translateText = () => {
    if (!inputText.trim()) { setTranslatedSigns([]); return; }
    setIsTranslating(true); stopSpeaking();
    setTimeout(() => {
      const words = inputText.toLowerCase().split(/\s+/).filter((word) => word.length > 0);
      const availableSigns = words.map((word) => word.replace(/[.,!?;:'"()]/g, "")).filter((cleanWord) => cleanWord in mockSignImages);
      setTranslatedSigns(availableSigns);
      if (inputText.trim() && !recentTranslations.includes(inputText.trim())) { setRecentTranslations((prev) => [inputText.trim(), ...prev].slice(0, 5)); }
      setIsTranslating(false); textareaRef.current?.focus();
    }, 800);
  };
        ...
        <div className="container px-4 md:px-6 relative max-w-screen-xl mx-auto">
          <div className="flex flex-col items-center justify-center space-y-4 text-center">
            <div className="space-y-2">
              <h1 className="heading-1 gradient-text-hero">{isLoading ? fallbacks.title : <span suppressHydrationWarning>{t("translator.title") ?? fallbacks.title}</span>}</h1>
              <p className="max-w-[700px] mx-auto text-muted-foreground md:text-xl/relaxed">{isLoading ? fallbacks.subtitle : <span suppressHydrationWarning>{t("translator.subtitle") ?? fallbacks.subtitle}</span>}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container px-4 md:px-6 max-w-screen-xl mx-auto">
          <div className="grid gap-8 lg:grid-cols-3 items-start lg:gap-12">
            <div className="lg:col-span-1 space-y-6">
              <div className="space-y-2">
                <h2 className="heading-2 gradient-text-heading">{isLoading ? fallbacks.inputTitle : <span suppressHydrationWarning>{t("translator.input.title") ?? fallbacks.inputTitle}</span>}</h2>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                   <Button variant="outline" size="sm" className="text-xs flex items-center gap-1" onClick={() => setHighContrastMode(!highContrastMode)} aria-pressed={highContrastMode}> {highContrastMode ? <EyeOff className="h-3 w-3" /> : <Eye className="h-3 w-3" />} {highContrastMode ? (isLoading ? fallbacks.standardMode : t("common.standardMode") ?? fallbacks.standardMode) : (isLoading ? fallbacks.highContrast : t("common.highContrast") ?? fallbacks.highContrast)} </Button>
                   ...
                   <Button variant="outline" size="sm" className={cn("text-xs flex items-center gap-1", isSpeaking && "bg-primary/10 text-primary border-primary")} onClick={() => isSpeaking ? stopSpeaking() : speakText(isLoading? fallbacks.readPageInstruction : t("translator.readPageInstruction") ?? fallbacks.readPageInstruction)}> {isSpeaking ? <RefreshCw className="h-3 w-3 animate-spin" /> : <Volume2 className="h-3 w-3" /> } {isSpeaking ? (isLoading? fallbacks.stop : t("common.stop") ?? fallbacks.stop) : (isLoading? fallbacks.readPage : t("common.readPage") ?? fallbacks.readPage)} </Button>
                </div>
              </div>
              <div className="space-y-4">
                 <Textarea ref={textareaRef} placeholder={isLoading? fallbacks.inputPlaceholder : t("translator.input.placeholder") ?? fallbacks.inputPlaceholder} className={cn("w-full rounded-md border border-input min-h-[120px] p-3 ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 transition-colors", highContrastMode ? "bg-black text-white placeholder:text-gray-400 border-white" : "bg-background placeholder:text-muted-foreground")} style={{ fontSize: `${fontSize}px` }} value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={handleKeyDown} rows={5} />
                 <div className="flex gap-2">
                   <Button onClick={translateText} disabled={isTranslating || !inputText.trim()} className="flex-1 btn-gradient" aria-live="polite"> {isTranslating ? (<><RefreshCw className="mr-2 h-4 w-4 animate-spin" />{isLoading ? fallbacks.translating : <span suppressHydrationWarning>{t("translator.translating") ?? fallbacks.translating}</span>}</>) : (<><Send className="mr-2 h-4 w-4" />{isLoading ? fallbacks.translateButton :<span suppressHydrationWarning>{t("translator.button") ?? fallbacks.translateButton}</span>}</>)}</Button>
                   <Button onClick={() => isSpeaking ? stopSpeaking() : speakText(inputText)} disabled={!inputText.trim()} variant="outline" className="btn-secondary" aria-label={isSpeaking ? "Stop speaking" : "Speak text"}> {isSpeaking ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Volume2 className="h-4 w-4" />} </Button>
                 </div>
               </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="space-y-2">
                ...
                {translatedSigns.length > 0 && ( <Button variant="outline" size="sm" className={cn("mt-2 text-xs flex items-center gap-1", isSpeaking && "bg-primary/10 text-primary border-primary")} onClick={() => isSpeaking ? stopSpeaking() : speakText(`Signs: ${translatedSigns.join(", ")}`)}><span className="sr-only">Read sign names aloud</span>{isSpeaking ? <RefreshCw className="h-3 w-3 animate-spin" /> : <Volume2 className="h-3 w-3" />} {isSpeaking ? (isLoading? fallbacks.stop : t("common.stop") ?? fallbacks.stop) : (isLoading? fallbacks.readSigns : t("translator.readSigns") ?? fallbacks.readSigns)}</Button> )}
              </div>
                  ...
                  {!isTranslating && translatedSigns.length > 0 && (
                    <div className="flex flex-wrap gap-3 sm:gap-4 justify-center">
                      {translatedSigns.map((sign, index) => (
                        <div key={`${sign}-${index}`} className={cn("text-center flex flex-col items-center space-y-1.5 p-2 rounded-md border shadow-sm transition-colors", highContrastMode ? "bg-black text-white border-white" : "bg-background border-border/60")}>
                          ...
                          <span className={cn("capitalize font-medium transition-colors", highContrastMode ? "text-white" : "text-muted-foreground", fontSize > 16 ? "text-sm" : "text-xs")}>{sign}</span>
                        </div>
                      ))}
                    </div>
                  )}
         ...
         <div className="container px-4 md:px-6 max-w-screen-xl mx-auto">
           <div className="text-center mb-10">
             ...
             <div className="flex flex-col sm:flex-row gap-4 justify-center mt-6">
                <Link href="/learn" scroll={true}> <Button size="lg" className="btn-gradient group">{isLoading ? fallbacks.learnCta : <span suppressHydrationWarning>{t("translator.learn.cta") ?? fallbacks.learnCta}</span>}<ChevronRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" /></Button></Link>
                <Link href="/family-orientation" scroll={true}><Button size="lg" variant="outline" className="btn-secondary">{isLoading ? fallbacks.familyCta : <span suppressHydrationWarning>{t("translator.family.cta") ?? fallbacks.familyCta}</span>}</Button></Link>
              </div>
           </div>
