...
const fallbacks = {
    ...
    stepLearnLongDesc: "Our comprehensive curriculum covers everything from basic signs to complex conversations in Ghanaian Sign Language.",
    stepLearnCta: "View Lessons",
    stepFamilyTitle: "Family Orientation",
    stepFamilyDesc: "Support for families with deaf members",
    stepFamilyLongDesc: "Resources and guidance for families to create inclusive environments and effectively communicate with deaf family members.",
    stepFamilyCta: "Family Resources",
    stepCareerTitle: "Career Aid",
    stepCareerDesc: "Professional development for GSL users",
    stepCareerLongDesc: "Career resources, job opportunities, and professional development for GSL users and interpreters.",
...
export default function Home() {
  ...
  const steps = [
    { id: 1, icon: BookOpen, titleKey: "home.steps.learn.title", descriptionKey: "home.steps.learn.description", longDescriptionKey: "home.steps.learn.longDescription", linkTextKey: "home.steps.learn.cta", linkHref: "/learn", fallbackTitle: fallbacks.stepLearnTitle, fallbackDesc: fallbacks.stepLearnDesc, fallbackLongDesc: fallbacks.stepLearnLongDesc, fallbackLinkText: fallbacks.stepLearnCta },
    { id: 2, icon: Users, titleKey: "home.steps.family.title", descriptionKey: "home.steps.family.description", longDescriptionKey: "home.steps.family.longDescription", linkTextKey: "home.steps.family.cta", linkHref: "/family-orientation", fallbackTitle: fallbacks.stepFamilyTitle, fallbackDesc: fallbacks.stepFamilyDesc, fallbackLongDesc: fallbacks.stepFamilyLongDesc, fallbackLinkText: fallbacks.stepFamilyCta },
    { id: 3, icon: Briefcase, titleKey: "home.steps.career.title", descriptionKey: "home.steps.career.description", longDescriptionKey: "home.steps.career.longDescription", linkTextKey: "home.steps.career.cta", linkHref: "/careers-scholarships", fallbackTitle: fallbacks.stepCareerTitle, fallbackDesc: fallbacks.stepCareerDesc, fallbackLongDesc: fallbacks.stepCareerLongDesc, fallbackLinkText: fallbacks.stepCareerCta }
  ];
    ...
    <>
        ...
        <div className="container px-4 md:px-6 relative max-w-screen-xl mx-auto">
          <div className="flex flex-col items-center justify-center space-y-4 text-center">
            <div className="space-y-2 mx-auto text-center">
              ...
              <h1 className="heading-1 lg:text-6xl gradient-text-hero mx-auto text-center">{isLoading ? fallbacks.heroTitle : <span suppressHydrationWarning>{t("home.hero.title") ?? fallbacks.heroTitle}</span>}</h1>
          ...
          <div className="mt-12 flex justify-center">
            <div className="relative w-full max-w-md mx-auto lg:max-w-lg">
              ...
              <Image src="/placeholder.svg?height=400&width=600" width={600} height={400} alt={isLoading ? fallbacks.heroImageAlt : t("home.hero.imageAlt") ?? fallbacks.heroImageAlt} className="rounded-xl relative shadow-lg object-cover border-4 border-background rotate-3 w-full h-auto img-duotone-pink-purple" priority />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container px-4 md:px-6 max-w-screen-xl mx-auto">
          <div className="flex flex-col items-center justify-center space-y-4 text-center mb-10 lg:mb-12">
            <div className="space-y-2 max-w-3xl">
              <h2 className="heading-2 gradient-text-heading">{isLoading ? fallbacks.featuredLearningTitle : <span suppressHydrationWarning>{t("home.featuredLearning.title") ?? fallbacks.featuredLearningTitle}</span>}</h2>
              <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed text-enhanced">{isLoading ? fallbacks.featuredLearningSubtitle : <span suppressHydrationWarning>{t("home.featuredLearning.subtitle") ?? fallbacks.featuredLearningSubtitle}</span>}</p>
            </div>
          </div>
        ...
        <div className="container px-4 md:px-6 max-w-screen-xl mx-auto">
          <div className="flex flex-col items-center justify-center space-y-4 text-center mb-10 lg:mb-12">
            <div className="space-y-2 max-w-3xl mx-auto text-center">
              <h2 className="heading-2 gradient-text-heading mx-auto text-center">{isLoading ? fallbacks.whyTitle : <span suppressHydrationWarning>{t("home.why.title") ?? fallbacks.whyTitle}</span>}</h2>
              <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed text-enhanced">{isLoading ? fallbacks.whyDesc : <span suppressHydrationWarning>{t("home.why.desc") ?? fallbacks.whyDesc}</span>}</p>
            </div>
          </div>
              ...
              <ul className="grid gap-3">
                {[1, 2, 3, 4].map(num => (
                  <li key={num} className="flex items-start gap-3 purple-accent">
                    ...
                    <span className="text-base text-muted-foreground text-enhanced">{isLoading ? fallbacks[`whyPoint${num}` as keyof typeof fallbacks] : <span suppressHydrationWarning>{t(`home.why.point${num}`) ?? fallbacks[`whyPoint${num}` as keyof typeof fallbacks]}</span>}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="container px-4 md:px-6 max-w-screen-xl mx-auto">
          <div className="grid gap-10 lg:grid-cols-2 lg:gap-12 items-center">
            ...
            <div className="space-y-4 lg:space-y-5 order-1 lg:order-2">
              <h2 className="heading-2 gradient-text-heading">{isLoading ? fallbacks.translatorTitle : <span suppressHydrationWarning>{t("home.translator.title") ?? fallbacks.translatorTitle}</span>}</h2>
              <p className="text-muted-foreground md:text-lg/relaxed text-enhanced">{isLoading ? fallbacks.translatorDesc : <span suppressHydrationWarning>{t("home.translator.desc") ?? fallbacks.translatorDesc}</span>}</p>
              <ul className="grid gap-3">
                 {[1, 2, 3].map(num => (
                  <li key={num} className="flex items-start gap-3 purple-accent">
                    <span className="mt-1 flex h-2 w-2 translate-y-1 rounded-full bg-purple-500 flex-shrink-0 glow-purple" />
                    <span className="text-base text-muted-foreground text-enhanced">{isLoading ? fallbacks[`translatorPoint${num}` as keyof typeof fallbacks] : <span suppressHydrationWarning>{t(`home.translator.point${num}`) ?? fallbacks[`translatorPoint${num}` as keyof typeof fallbacks]}</span>}</span>
                  </li>
                 ))}
              </ul>
              <div className="pt-4">
                <Link href="/translator" scroll={true}><Button className="btn-gradient px-6">{isLoading ? fallbacks.translatorCta : <span suppressHydrationWarning>{t("home.translator.cta") ?? fallbacks.translatorCta}</span>}</Button></Link>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="featured-content" className="section-padding content-bg-1">
        <div className="container px-4 md:px-6 max-w-screen-xl mx-auto">
          <div className="flex flex-col items-center justify-center space-y-4 text-center mb-10 lg:mb-12">
            <div className="space-y-2 max-w-3xl mx-auto text-center">
              <h2 className="heading-2 gradient-text-heading mx-auto text-center">{isLoading ? fallbacks.featuredContentTitle : <span suppressHydrationWarning>{t("home.featuredContent.title") ?? fallbacks.featuredContentTitle}</span>}</h2>
              <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed text-enhanced">{isLoading ? fallbacks.featuredContentDesc : <span suppressHydrationWarning>{t("home.featuredContent.desc") ?? fallbacks.featuredContentDesc}</span>}</p>
            </div>
          </div>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 lg:gap-8">
             <Card className="card-standard glass-card-content gradient-border subtle-shadow shine-effect">
               <CardHeader className="p-0">
                 ...
                 <div className="p-4 md:p-5"><CardTitle className="text-lg font-semibold group-hover:text-pink-600 transition-colors">{isLoading ? fallbacks.featuredGreetingsTitle : <span suppressHydrationWarning>{t("home.featuredContent.greetings.title") ?? fallbacks.featuredGreetingsTitle}</span>}</CardTitle><CardDescription className="text-sm mt-1">{isLoading ? fallbacks.featuredGreetingsDesc : <span suppressHydrationWarning>{t("home.featuredContent.greetings.description") ?? fallbacks.featuredGreetingsDesc}</span>}</CardDescription></div>
               </CardHeader>
             ...
             <Card className="card-standard glass-card-content gradient-border subtle-shadow shine-effect">
               <CardHeader className="p-0">
                  ...
                  <div className="p-4 md:p-5"><CardTitle className="text-lg font-semibold group-hover:text-pink-600 transition-colors">{isLoading ? fallbacks.featuredFamilyTitle : <span suppressHydrationWarning>{t("home.featuredContent.family.title") ?? fallbacks.featuredFamilyTitle}</span>}</CardTitle><CardDescription className="text-sm mt-1">{isLoading ? fallbacks.featuredFamilyDesc : <span suppressHydrationWarning>{t("home.featuredContent.family.description") ?? fallbacks.featuredFamilyDesc}</span>}</CardDescription></div>
                </CardHeader>
             ...
             <Card className="card-standard glass-card-content gradient-border subtle-shadow shine-effect">
                <CardHeader className="p-0">
                    ...
                    <div className="p-4 md:p-5"><CardTitle className="text-lg font-semibold group-hover:text-pink-600 transition-colors">{isLoading ? fallbacks.featuredNumbersTitle : <span suppressHydrationWarning>{t("home.featuredContent.numbers.title") ?? fallbacks.featuredNumbersTitle}</span>}</CardTitle><CardDescription className="text-sm mt-1">{isLoading ? fallbacks.featuredNumbersDesc : <span suppressHydrationWarning>{t("home.featuredContent.numbers.description") ?? fallbacks.featuredNumbersDesc}</span>}</CardDescription></div>
                </CardHeader>
          ...
          <div className="flex justify-center mt-8">
            <Link href="/learn"><Button variant="outline" className="btn-secondary">{isLoading ? fallbacks.featuredViewAll : <span suppressHydrationWarning>{t("home.featuredContent.viewAll") ?? fallbacks.featuredViewAll}</span>} <ChevronRight className="ml-1 h-4 w-4" /></Button></Link>
          </div>
        </div>
      </section>
      ...
      <section id="join-community" className="section-padding cta-bg">
        <div className="container px-4 md:px-6 relative max-w-screen-xl mx-auto">
          <div className="flex flex-col items-center justify-center space-y-4 text-center">
            <div className="space-y-2 max-w-3xl mx-auto text-center">
              <h2 className="heading-2 text-white mx-auto text-center">{isLoading ? fallbacks.joinCommunityTitle : <span suppressHydrationWarning>{t("home.joinCommunity.title") ?? fallbacks.joinCommunityTitle}</span>}</h2>
              <p className="max-w-[900px] text-white/80 md:text-xl/relaxed">{isLoading ? fallbacks.joinCommunityDesc : <span suppressHydrationWarning>{t("home.joinCommunity.desc") ?? fallbacks.joinCommunityDesc}</span>}</p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center pt-4 max-w-md mx-auto">
                 <div className="flex w-full items-center space-x-2 rounded-lg border border-white/20 bg-white/10 p-1 pr-1.5 h-12">
                   <Input type="email" placeholder={isLoading ? fallbacks.joinCommunityPlaceholder : t("home.joinCommunity.placeholder") ?? fallbacks.joinCommunityPlaceholder} className="flex-grow h-full border-none bg-transparent text-white placeholder:text-white/60 focus:ring-0 focus:outline-none px-3"/>
                   <Button className="h-full shrink-0 rounded-md bg-white px-3 text-purple-600 hover:bg-white/90 text-sm">{isLoading ? fallbacks.joinCommunityButton : <span suppressHydrationWarning>{t("home.joinCommunity.button") ?? fallbacks.joinCommunityButton}</span>}</Button>
                 </div>
              </div>
